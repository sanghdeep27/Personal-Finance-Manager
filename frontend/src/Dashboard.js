...
export default function PersonalFinanceDashboard() {
  ...
  const handleAddLoan = () => {
    ...
    setLoans([...loans, { amount, rate, tenure: months, interest, total: amount + interest }]);
    setLoanAmount("");
    setInterestRate("");
    setTenure("");
  };
  const markAsPaid = (index) => {
    setLoans(loans.map((loan, i) => i === index ? { ...loan, paid: true } : loan));
  };
  ...
  useEffect(() => {
    ...
    const upcomingBills = bills.filter(bill => new Date(bill.dueDate) <= today);
    if (upcomingBills.length) {
      alert("You have upcoming bill payments!");
    }
  }, [bills]);
  ...
  const addBillReminder = () => {
    ...
    setBills([...bills, { name: billName, amount: billAmount, dueDate: billDueDate }]);
    setBillName("");
    setBillAmount("");
    setBillDueDate("");
  };
                  ...
                  <tr key={index}>
                    ...
                    <td>
                      {loan.paid ? (
                        ...
                        <button onClick={() => markAsPaid(index)} className="mark-paid-button">Mark as Paid</button>
                      )}
                    </td>
                  </tr>
                ))}
        ...
        {activePanel === "bills" && (
          <div className="bills-panel">
            ...
            <div className="bill-inputs">
              <input type="text" placeholder="Bill Name" value={billName} onChange={(e) => setBillName(e.target.value)} className="input-field" />
              <input type="number" placeholder="Amount" value={billAmount} onChange={(e) => setBillAmount(e.target.value)} className="input-field" />
              <input type="date" value={billDueDate} onChange={(e) => setBillDueDate(e.target.value)} className="input-field" />
              <button onClick={addBillReminder} className="add-bill-button">Add Bill</button>
            </div>
